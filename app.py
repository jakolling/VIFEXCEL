...
if all([wyscout_file, physical_file, overcome_file]):  
    ...
    # Consolidate Skillcorner players from both datasets; filter out players already matched  
    ...
    if not st.session_state.auto_matched:  
        for player in wyscout_players:  
            if player not in st.session_state.temp_selections:  
                best_match = find_best_match(player, available_skillcorner_players)  
    ...
    with col1:  
        ...
        with col_metrics1:  
            ...
            selected_physical = st.multiselect("Physical Output metrics:", physical_cols, default=physical_cols)  
        with col_metrics2:  
            overcome_cols = [col for col in overcome_df.columns if col != "Player"]  
            selected_overcome = st.multiselect("Overcome Pressure metrics:", overcome_cols, default=overcome_cols)  
    with col2:  
        st.write("### Navigation")  
        players_per_page = 10  
        ...
        with col_nav1:  
            if st.button("◀ Previous") and st.session_state.page_number > 0:  
                ...
                st.rerun()  
        with col_nav2:  
            st.write("Page " + str(st.session_state.page_number + 1) + " of " + str(max(1, total_pages)))  
        with col_nav3:  
    ...
    for idx in range(start_idx, end_idx):  
        if idx < len(wyscout_players):  
            ...
            with cols[1]:  
                ...
                selection_index = get_selection_index(current_selection, available_skillcorner_players)  
                selection = st.selectbox("Match with:", ["-- None --"] + available_skillcorner_players,   
                                           index=selection_index, key="select_" + player)  
                st.session_state.temp_selections[player] = selection if selection != "-- None --" else None  
            with cols[2]:  
                if st.button("✓", key="confirm_" + player, help="Confirm match"):  
                    if selection and selection != "-- None --":  
                        ...
                        st.rerun()  
            with cols[3]:  
                if st.button("✗", key="reject_" + player, help="Reject player"):  
    ...
    with col2:  
        ...
        if st.button("📥 Export Data", help="Download matched data as Excel"):  
            ...
            merged_df = pd.merge(wyscout_matched,  
                                 physical_subset,  
            ...
            final_df = pd.merge(merged_df,  
                                ...
                                right_on="Player",  
                                how="left",  
                                suffixes=("", "_Overcome")).dropna(subset=["Player"])  
        ...
        st.write("Matched: " + str(len(st.session_state.confirmed_matches)) + " of " +  
                 str(len(wyscout_df["Player"].dropna().unique())) + " players")  
          
        st.write("### Rejected Players")  
        if st.session_state.rejected_players:  
            st.write("Total rejected: " + str(len(st.session_state.rejected_players)))  
