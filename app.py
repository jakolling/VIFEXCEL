...
def main():  
    ...
    1. Faça upload dos arquivos Excel:  
       - Database WyScout  
       - Physical Output (SkillCorner)  
       - Overcome Pressure (SkillCorner)  
    2. O sistema fará o match automático dos jogadores  
    3. Resolva manualmente apenas os mismatches identificados  
    4. As colunas de métricas do SkillCorner serão renomeadas com identificadores claros  
    5. Baixe o arquivo Excel mesclado final  
    """)  
      
    wyscout_file = st.file_uploader("Database WyScout", type=["xlsx"])  
    physical_file = st.file_uploader("Physical Output (SkillCorner)", type=["xlsx"])  
    pressure_file = st.file_uploader("Overcome Pressure (SkillCorner)", type=["xlsx"])  
    ...
    if all([wyscout_file, physical_file, pressure_file]):  
        try:  
            ...
            # Renomear as colunas dos dataframes de SkillCorner com prefixos identificadores  
            physical_prefix = "Physical_Output_"  
            pressure_prefix = "Overcome_Pressure_"  
              
            physical_cols = {col: physical_prefix + col for col in df_physical.columns if col != "Player"}  
            pressure_cols = {col: pressure_prefix + col for col in df_pressure.columns if col != "Player"}  
              
            df_physical.rename(columns=physical_cols, inplace=True)  
            df_pressure.rename(columns=pressure_cols, inplace=True)  
