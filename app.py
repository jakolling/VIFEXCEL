...
def main():  
    ...
    if all([wyscout_file, physical_file, pressure_file]):  
        try:  
            ...
            # Adicionar prefixo nas colunas para identificar origem  
            physical_cols = [col for col in df_physical.columns if col != "Player"]  
            pressure_cols = [col for col in df_pressure.columns if col != "Player"]  
              
            df_physical = df_physical.rename(columns={col: f"Physical_{col}" for col in physical_cols})  
            df_pressure = df_pressure.rename(columns={col: f"Pressure_{col}" for col in pressure_cols})  
